...
public class SingleProductAdapter extends RecyclerView.Adapter<SingleProductAdapter.ViewHolder> {
    ...
    private List<SingleProduct> SingleProductAdapters;
    private SingleProductActivity mCtx;
    private RecyclerView recyclerView = null;
    int previousExpandedPosition = -1;
    int mExpandedPosition = -1;

    public SingleProductAdapter(List<SingleProduct> products, SingleProductActivity mCtx) {
        this.SingleProductAdapters = products;
        this.mCtx = mCtx;
    ...
    @Override
    public SingleProductAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_product_items_temp_2, parent, false);
        return new SingleProductAdapter.ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(final SingleProductAdapter.ViewHolder holder, final int position) {
        final SingleProduct product = SingleProductAdapters.get(position);
        ...
        if(product.getDiscount_price().equals("0.0000"))
        {
            ...
            holder.textViewPrice.setTextColor(ContextCompat.getColor(mCtx, R.color.colorPrimary1));
            holder.textViewPrice.setText(new StringBuilder(mCtx.getResources().getString(R.string.currency_sign)).append(df2.format(price)));
        ...
        else
        {
            ...
            else
            {
                ...
                holder.textViewPrice.setTextColor(ContextCompat.getColor(mCtx, R.color.colorPrimary));
                holder.textViewPrice.setText(new StringBuilder(mCtx.getResources().getString(R.string.currency_sign)).append(df2.format(price)));
                ...
                holder.textViewDiscount.setText(new StringBuilder(mCtx.getResources().getString(R.string.currency_sign)).append(df2.format(discountPrice)));
            }
        }
        Picasso.with(mCtx)
        ...
        if (new DatabaseMedicine(mCtx).checkExistence(product.getProduct_id())) {
            holder.buttonAddToCart.setBackgroundResource(R.color.colorPrimary1);
            String quantity = new DatabaseMedicine(mCtx).countQuantity(product.getProduct_id(), product.getCategory_id());
                ...
                public boolean onLongClick(View v) {
                    ...
                    alertDialog.setNegativeButton("REMOVE", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            new DatabaseMedicine(mCtx).clearCartITemFromProcut(product.getProduct_id(), product.getCategory_id());
        ...
        holder.buttonAddToCart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Log.d("Cat_ID: " , product.getCategory_id());                // Checking if the product is already added or not
                if (!new DatabaseMedicine(mCtx).checkExistence(product.getId())) {

                    if(product.getDiscount_price().equals("0.0000"))
                    {
                        new DatabaseMedicine(mCtx).addToCart(new Cart(
                    ...
                    Toast.makeText(mCtx, "Long press to edit selection", Toast.LENGTH_SHORT).show();
                }
            }
        });
                ...
                call.enqueue(new Callback<Result>() {
                    ...
                    public void onResponse(Call<Result> call, Response<Result> response) {
                        ...
                        else
                        {
                            ...
                            Toast.makeText(mCtx, product.getName() + " was removed from wishlist", Toast.LENGTH_SHORT).show();
                        }
                    }
                    @Override
                    public void onFailure(Call<Result> call, Throwable t) {
        ...
        holder.imageView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Common.submenu_name = product.getCat_name();
                if(product.getDiscount_price().equals("0.0000"))
                {
                ...
                Common.singleProduct = SingleProductAdapters.get(position);
                mCtx.startActivity(new Intent(mCtx, ProductActivity.class));
            }
        });
    }

    private void checkExistance(final ViewHolder holder, String product_id) {
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(mCtx);
    ...
    private void removeFromWishlist(final ViewHolder holder, String customer_id, String product_id) {
    ...
    public int getItemCount() {
        ...
        return (null != SingleProductAdapters ? SingleProductAdapters.size() : 0);
    }
    ...
    public void viewCart() {
        // Test
        final int count = new DatabaseMedicine(mCtx).cartItemCount();

        if (count > 0) {
            mCtx.layoutViewCart.setVisibility(View.VISIBLE);

            /*if (count == 1) {
                mCtx.textViewItems.setText(count + " item in cart");
            } else {
                mCtx.textViewItems.setText(count + " items in cart");
            }*/

            //Update Total Price
            double total = 0.0;
            List<Cart> carts = new DatabaseMedicine(mCtx).getCarts();
            for (Cart cart : carts) {
                total += (Double.parseDouble(cart.getPrice())) * (Double.parseDouble(cart.getQuantity()));
            }
            mCtx.textViewPrice.setText(mCtx.getResources().getString(R.string.currency_sign) + String.format("%.2f", total));

            mCtx.layoutCart.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    mCtx.startActivity(new Intent(mCtx, MedicineCartActivity.class));
                    mCtx.finish();

                }
            });
        } else {
            mCtx.layoutViewCart.setVisibility(View.GONE);
        }
    }
}